...
//        var children:Array = this.getChildren();
        ...
        {
			var child:Object = this.getElementAt(index);
            if (child is IWidgetTemplate)
            {
                _widgetTemplate = child as IWidgetTemplate;

                _widgetTemplate.baseWidget = this as IBaseWidget;
...
//        {
	...
	/**
	 * 获取其他兄弟widget的Id
	 * @param url 目标widget的url
	 * @return 目标widget的id, 未发行匹配的，返回-1
	 */
	public function siblingWidgetId(url:String):int
	{
		for each(var widget:Object in configData.widgets)
		{
			if(widget.url == url)
			{
				return parseInt(widget.id.toString());
			}
		}
		return -1;
	}
	...
	/**
	 * 获取widgetContainer的Id
	 * @param url 目标widget的url
	 * @return 目标widget的id, 未发行匹配的，返回-1
	 */
	public function widgetContainerId(url:String):int
	{
		for each(var container:Object in configData.widgetContainers)
		{
			if(container.container.url == url)
			{
