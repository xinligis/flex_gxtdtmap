...
import com.esri.viewer.remote.RemoteObject;
...
public class ConfigManager extends EventDispatcher
{
    ...
    private function configService_resultHandler(event:ResultEvent):void
    {
        try
        {
            ...
            //Style configuration
            //================================================
            var styleAlpha:String = (configXML.style.alpha[0] != null) ? configXML.style.alpha : configXML.stylealpha;
            if (styleAlpha)
            {
			...
			//==============================================
			//BlazeDS中所需的AMFChannel的配置信息，如果不存在，则使用默认
			//==============================================
			if(configXML.amfchannel[0])
			{
				var uri:String = configXML.amfchannel[0].@uri || "";
				if(uri != "")
				{
					RemoteObject.channelUri = uri;
				}
			}
            //================================================
            ...
            if (arcGISWebMapItemID)
            {
                ...
                if (arcgisSharingURL)
                {
                    webMapUtil.portalURL = arcgisSharingURL;
                }
            ...
            else
            {
				...
				var configBasegroup:Array=[];
				var basemapGroup:String;
				var basemaps:XMLList=configXML.map.basemaps;
				var configMap:Array = [];
				var baseNum:int = 0;
				var configBasemaps:Array = [];
				for( var t:Number=0;t<basemaps.length();t++)
				{
					
					var maplayerList:XMLList = configXML.map.basemaps[t].layer;
					basemapGroup = configXML.map.basemaps[t].@group;
					var visible:Boolean = configXML.map.basemaps[t].@visible == "true";
					
					//读取配置文件中的地图切换按钮的图标，如果没有配置则为空；温杨彪；2011/7/22
					if(configXML.map.basemaps[t].@unselectedIcon[0] && configXML.map.basemaps[t].@selectedIcon[0])
					{
						configBasegroup.push({label:basemapGroup,
							visible:visible,
							unselectedIcon:configXML.map.basemaps[t].@unselectedIcon[0].toString(),
							selectedIcon:configXML.map.basemaps[t].@selectedIcon[0].toString()});
					}
					else
					{
						configBasegroup.push({label:basemapGroup,visible:visible});
					}
					for (var k:int=0;k<maplayerList.length();k++)
					{
						configBasemaps.push(getLayerObject(maplayerList[k], baseNum + k, false, bingKey,null,basemapGroup));
					}
					//在group中的各个layer的序号依次累加，并作为各个layer的id
					baseNum += maplayerList.length();
				}
				configData.grouplayers = configBasegroup;
                
//                maplayerList = configXML.map.basemaps.mapservice; // TODO - is this still in use ???
            ...
            function createWidgets(widgetList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
            {
                ...
                for (var p:int = 0; p < widgetListLength; p++)
                {
					...
					var wShowIcon:String = widgetList[p].@showicon || "true"; //是否在Header中显示功能图表，王红亮，2012-01-09
                    ...
                    var widget:Object =
                        {
                            ...
                            bottom: wBottom,
                            url: wUrl,
							showicon:wShowIcon //显示功能图标，王红亮，2012-01-09
                        };
            ...
            for (i = 0; i < wContainers.length(); i++)
            {
				...
				var wcIndex:String = wContainers[i].@index; //如果是拆分布局窗口，则其索引值
				var wcWidth:String = wContainers[i].@width || ""; //初始宽度，适用于拆分窗口,王红亮，2012-01-09
				var wcVisible:String = wContainers[i].@visible || "true"; //初始时是否可将，默认为true，王红亮，2012-01-09
                if (!wcLayout)
                {
			...
			//urlOperations；温杨彪；2012-4-20
			if(configXML.urlopertations.operation)
			{
				var urlOpers:Hashtable = new Hashtable();
				var operations:XMLList = configXML.urlopertations.operation;
				for each(var oper:XML in operations)
				{
					urlOpers.add(String(oper.@id),String(oper.@url));
				}
				if(urlOpers.size>0)
				{
					configData.urlOperation = urlOpers;
				}
			}
			
			//解析URL查询字符串
			configData.urlParam = parseUrlParams();
//			configData.urlParam = {operation:"marker",data:""};
//			configData.urlParam = {operation:"drive",start:"西",end:"重庆"};
//			configData.urlParam = {operation:"bus",start:"大庙",end:"航天职大"};
//			configData.urlParam = {operation:"search",type:"4"};
	...
	public function parseUrlParams():Object
	{
		...
		if (query)   
		{
			...
			for (var i:uint = 0 ; i  <  pairs.length; i ++ )   
			{
				...
				if (pos  !=   - 1 )   
				{
					...
					params[argname]  =  decodeURI(value);
//					Alert.show("key:"+argname+",value:"+value);
				} 
			} 
		}
...
//		}
    ...
    private function getLayerObject(obj:XML, num:Number, isOpLayer:Boolean, bingKey:String, layer:Layer = null,group:String=""):Object
    {
		...
		//WMTS图层的特殊信息，2013-3-22
		var wmtsLayer:String = obj.@wmtslayer[0] ? obj.@wmtslayer[0] : "";
		var tileMatrixSet:String = obj.@tileMatrixSet[0] ? obj.@tileMatrixSet[0] : "";
		var format:String = obj.@format[0] ? obj.@format[0] : "";		
        // ve tiled layer
